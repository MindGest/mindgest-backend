...
export async function statistics(req: Request<{}, {}, {}, QueryStatistics>, res: Response){
    ...
    if(query.startDate==null || query.endDate == null){
        ...
        appointments = await prisma.$queryRaw<appointment[]>`Select * from appointment where extract(month from slot_start_date) >= ${parseInt(monthStart)} and extract(year from slot_start_date) >= ${parseInt(yearStart)} and extract(day from slot_start_date) >= ${parseInt(dayStart)}
        INTERSECT
        Select * from appointment where extract(month from slot_end_date) <= ${parseInt(monthEnd)} and extract(year from slot_end_date) <= ${parseInt(yearEnd)} and extract(day from slot_end_date) <= ${parseInt(dayEnd)}`
    ...
    for(let appointment of appointments){
        ...
        if(query.processId == null || (query.processId!=null && processAppointment?.process_id!.toString() == query.processId!)){
            ...
            if(query.specialityId == null || (query.specialityId!=null && speciality==query.specialityId)){
                    ...
                    for(let therapist of therapistsProcess){
                        ...
                        if(query.therapistId! == therapistInfo?.id.toString()){
                            ...
                            if(therapistsApointments.has(therapistInfo!.name)){
                                therapistsApointments.set(therapistInfo!.name, therapistsApointments.get(therapistInfo!.name)!+1)
                            }
                            else{
                        ...
                        if(flag){
                            ...
                            if(processAppointments.has(`${process!.id}`)){
                                processAppointments.set(`${process!.id}`, processAppointments.get(`${process!.id}`)!+1)
                            }
                            else{
