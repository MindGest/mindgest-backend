...
async function updateInfoTherapist(
    req: Request,
    res: Response,
    userType: String
) {
    ...
    prisma.therapist.update({
        data: {
            ...
            person: {
                update: {
                    active:
                        body.active == EMPTY_FIELD
                            ? oldData.person.active
                            : body.active,
                    address:
                        body.address == EMPTY_FIELD
                            ? oldData.person.address
                            : body.address,
                    name:
                        body.name == EMPTY_FIELD
                            ? oldData.person.name
                            : body.name,
                    email:
                        body.email == EMPTY_FIELD
                            ? oldData.person.email
                            : body.email,
                    aproved:
                        body.aproved == EMPTY_FIELD
                            ? oldData.person.aproved
                            : body.aproved,
                    birth_date:
                        body.birth_date == EMPTY_FIELD
                            ? oldData.person.birth_date
                            : body.birth_date,
                    password:
                        body.password == EMPTY_FIELD
                            ? oldData.person.password
                            : body.password, // ISTO FAZ SEQUER SENTIDO?
                    phone_number:
                        body.phone_number == EMPTY_FIELD
                            ? oldData.person.phone_number
                            : body.phone_number,
                },
            },
        },
...
async function updateInfoAccountant(
    req: Request,
    res: Response,
    userType: String
) {
...
async function updateInfoPatient(
    req: Request,
    res: Response,
    userType: String
) {
    ...
    prisma.patient.update({
        data: {
            tax_number:
                body.tax_number == EMPTY_FIELD
                    ? oldData.tax_number
                    : body.tax_number,
            health_number:
                body.health_number == EMPTY_FIELD
                    ? oldData.health_number
                    : body.health_number,
            request:
                body.request == EMPTY_FIELD ? oldData.request : body.request,
            remarks:
                body.remarks == EMPTY_FIELD ? oldData.remarks : body.remarks,
            patient_type: {
                update: {
                    id:
                        body.patienttype == EMPTY_FIELD
                            ? oldData.patien_type_id
                            : body.patienttype,
                },
            },
            person: {
    ...
    if (oldSchool) {
        prisma.school.update({
            ...
            data: {
                name:
                    body.school_name == EMPTY_FIELD
                        ? oldSchool.name
                        : body.school_name,
                grade:
                    body.school_grade == EMPTY_FIELD
                        ? oldSchool.grade
                        : body.school_grade,
            },
        })
    ...
    if (oldProfession) {
        prisma.profession.update({
            ...
            data: {
                name:
                    body.profession_name == EMPTY_FIELD
                        ? oldProfession.name
...
async function updateInfoPerson(req: Request, res: Response, userType: String) {
    ...
    prisma.person.update({
        data: {
            ...
            address:
                body.address == EMPTY_FIELD ? oldData.address : body.address,
            name: body.name == EMPTY_FIELD ? oldData.name : body.name,
            email: body.email == EMPTY_FIELD ? oldData.email : body.email,
            aproved:
                body.aproved == EMPTY_FIELD ? oldData.aproved : body.aproved,
            birth_date:
                body.birth_date == EMPTY_FIELD
                    ? oldData.birth_date
                    : body.birth_date,
            password:
                body.password == EMPTY_FIELD ? oldData.password : body.password, // ISTO FAZ SEQUER SENTIDO?
            phone_number:
                body.phone_number == EMPTY_FIELD
