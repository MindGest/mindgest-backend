...
export async function getAllAppointments(req: Request<{}, {}, AppointmentsList>, res: Response){
    ...
    */
    ...
    if ((callerRole == 'admin' || callerRole == 'guard') && req.body.filterId != -1){ // the processes of the specified filterId
    ...
    // get the processes
    var processes: any = []
    if ((callerRole == 'admin' || callerRole == 'guard') && req.body.filterId == -1){// all processes
        processes = await prisma.process.findMany()
    }
    else{ // either intern or therapist
        for (let i = 0; i < processIds.length; i++){
            processes.push(await prisma.process.findFirst({
    ...
    for (let i = 0; i < processes.length; i++){
        ...
        for (let e = 0; e < intern_process.length; e++){
            interns.push(await prisma.intern.findFirst({
        ...
        for (let e = 0; e < therapist_process.length; e++){
            therapists.push(await prisma.therapist.findFirst({
        ...
        for (let e = 0; e < patient_process.length; e++){
            patients.push(await prisma.patient.findFirst({
...
export async function createAppointment(req: Request<{}, {}, AppointmentCreate>, res: Response) {
    ...
    */
    ...
    if (callerRole == 'therapist'){ // check if the callerId is a therapist associated to the process
        ...
        if (!therapist_process){
            return res.status(StatusCodes.FORBIDDEN).json({
                message: "It appears that you do not have permission to create an appointment, due to the fact that you are not associated with the its process."
            })
        }
    }
    else if (callerRole == 'intern'){ // check if the callerId is an intern associated to the process
...
export async function editAppointment(req: Request<{}, {}, AppointmentEdit>, res: Response){
    ...
    if(!process){
        return res.status(StatusCodes.NOT_FOUND).json({
            message: "An critical error has occured. It seems that the process of this appointment does not exist."
        })
    }
    ...
    if (callerRole == 'therapist'){ // check if the callerId is a therapist associated to the process
        ...
        if (!therapist_process){
            return res.status(StatusCodes.FORBIDDEN).json({
                message: "It appears that you do not have permission to edit this appointment, due to the fact that you are not associated with the its process."
            })
        }
    }
    else if (callerRole == 'intern'){ // check if the callerId is an intern associated to the process
    ...
    else if (!(callerRole == 'admin')){ // you shouldn't be here!!!
        return res.status(StatusCodes.FORBIDDEN).json({
...
export async function archiveAppointment(req: Request<{}, {}, AppointmentArchive>, res: Response){
    ...
    else if (!(callerRole == 'admin')){ // if not a therapist, nor an intern, nor an admin -> you're not supposed to be here
        return res.status(StatusCodes.FORBIDDEN).json({
            message: "You do not have permission to edit this appointment."
        })
    }
...
export async function getAllActiveAppointments(req: Request<{}, {}, AppointmentsList>, res: Response){
    /**
...
export default { getAllAppointments, createAppointment, infoAppointment, editAppointment, archiveAppointment, getAllActiveAppointments}