...
export async function register(
    ...
    try {
        ...
        res.status(StatusCodes.CREATED).json({
            message:
                'The user was created successfully, verification token generated!',
            verification: token,
        })
    } catch (error) {
...
export async function login(req: Request<{}, {}, LoginBody>, res: Response) {
    try {
        ...
        // Authenticate user
        let authenticated = await argon2.verify(
            person.password,
            req.body.password
        )
        ...
        if (!person.aproved) {
            return res.status(StatusCodes.UNAUTHORIZED).json({
                message:
                    'The account is not approved. Contact an admin to solve this issue!',
            })
        }
    ...
    try {
        if (req.signedCookies || req.body) {
            ...
            if (decoded) {
                ...
                const { userRole, isAdmin } = await fetchPersonProperties(
                    person.id
                )
...
export async function verify(
    req: Request<{}, {}, VerifyAccountBody>,
    res: Response
) {
    try {
        // Fetch and decoded the verification token
        let decoded = verifyToken<VerificationToken>(req.body.verification)
